"""Implementasi `DcsSesiService` di atas PostgreSQL (SQLAlchemy 2.0, sinkron).

MENGGANTI `InMemoryDcsSesiService` TANPA mengubah kontrak Protocol — signature
method identik, sehingga router, skema, error envelope, dan test kontrak HTTP
tidak ikut berubah.

Whitelist field search dipakai ULANG dari `..services.sesi.SEARCHABLE_FIELDS`
...
from ...models import DcsSesiModel
...
def _sesi_field_map() -> FieldMap:
    return {
        "id": FieldSpec(column=DcsSesiModel.id),
        "periode": FieldSpec(column=DcsSesiModel.periode),
        "status": FieldSpec(column=DcsSesiModel.status),
        "created_at": FieldSpec(
            column=DcsSesiModel.created_at, order_column=DcsSesiModel.created_at
        ),
    }


def _to_read(rec: DcsSesiModel) -> DcsSesiRead:
...
class SqlDcsSesiService:
    """`DcsSesiService` berbasis PostgreSQL. Terikat pada satu `Session` per request."""

    def __init__(self, session: Session) -> None:
        self._s = session

    def _get_model(self, sesi_id: str) -> DcsSesiModel:
        rec = self._s.get(DcsSesiModel, sesi_id)
        if rec is None:
            raise NotFoundError(f"Sesi DCS '{sesi_id}' tidak ditemukan.")
        return rec

    def list(self, *, limit: int, offset: int) -> tuple[list[DcsSesiRead], int]:
        total = self._s.scalar(select(func.count()).select_from(DcsSesiModel)) or 0
        rows = self._s.scalars(
            select(DcsSesiModel)
            .order_by(DcsSesiModel.created_at.desc())
            .limit(limit)
            .offset(offset)
        ).all()
    ...
    def search(
        ...
        order_cols = order_by_columns(order, field_map) or [DcsSesiModel.created_at.desc()]
        total = self._s.scalar(select(func.count()).select_from(DcsSesiModel).where(cond)) or 0
        rows = self._s.scalars(
            select(DcsSesiModel).where(cond).order_by(*order_cols).limit(limit).offset(offset)
        ).all()
        return [_to_read(r) for r in rows], total
