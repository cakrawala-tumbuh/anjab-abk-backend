"""Test endpoint instrumen singleton WCP: get/update/tutup/buka-ulang."""
...
def _submit_satu_responden(client: TestClient, partisipan_factory, slug: str) -> str:
    """Assign + submit satu responden WCP; kembalikan `responden_id`. Mengisi seluruh
    item lewat dimensi (pola sama dengan `test_wcp_analisis.py`)."""
    par_id = partisipan_factory(slug)
    r = client.post("/api/v1/wcp/responden", json={"partisipan_ids": [par_id]})
    assert r.status_code == 201, r.text
    rsp_id = r.json()["created"][0]["id"]

    item_ids: list[str] = []
    dims = client.get("/api/v1/wcp/dimensi").json()
    for dim in dims:
        items = client.get(f"/api/v1/wcp/dimensi/{dim['kode']}/items").json()
        item_ids.extend(i["item_id"] for i in items)

    r2 = client.put(
        f"/api/v1/wcp/responden/{rsp_id}/jawaban",
        json={"jawaban": [{"item_id": iid, "skor_raw": 3} for iid in item_ids]},
    )
    assert r2.status_code == 200
    r3 = client.post(f"/api/v1/wcp/responden/{rsp_id}/jawaban/submit")
...
def test_reset_dari_analyzed_kembali_open_dan_responden_kosong(
    ...
    _submit_satu_responden(client, partisipan_factory, "wcp-reset-analyzed")
    client.post(f"{BASE}/tutup")
    r_analisis = client.post("/api/v1/wcp/analisis")
...
def test_reset_bukan_admin_ditolak(client_as) -> None:
    non_admin = client_as("wcp-reset-nonadmin", groups=["partisipan"])
...
def test_buka_ulang_biasa_tetap_ditolak_dari_analyzed(
    ...
    client.patch(BASE, json={"min_responden": 1})
    _submit_satu_responden(client, partisipan_factory, "wcp-reset-vs-bukaulang")
    client.post(f"{BASE}/tutup")
    client.post("/api/v1/wcp/analisis")
    assert client.get(BASE).json()["status"] == "ANALYZED"
